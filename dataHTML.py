...
def displayAllFeatures_HTML(filename, image_str, numImages):
    # Create an HTML template for displaying the images
    html_template = '''
    <!DOCTYPE html>
    <html>
    ...
    rows.append(f'<td>(L,a,b) = ({faceData[1]:.2f}, {faceData[2]}, {faceData[3]})</td>')
    bgrVal = faceData[0]
    rows.append(f'<td>(R,G,B) = ({bgrVal[2]},{bgrVal[1]},{bgrVal[0]})</td>')
...
imgage_str = "ColorCorrectedImages/CCF8.jpg"
displayAllFeatures_HTML("test", imgage_str, 1)
